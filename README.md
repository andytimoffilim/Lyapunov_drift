...
├── code/
│ ├── Doob_CSG.py # main experiment script (generates data and figures)
│ └── analysis.py # optional script to re‑plot figures from saved CSV
└── data/
...
- `code/Doob_CSG.py` – the main Python script that runs the full synthetic experiment, computes all indices, and generates the figures.
- `code/analysis.py` – a helper script that recreates the figures from the saved CSV (useful for quick plotting without re‑running the experiment).
